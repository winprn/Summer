...
int main() {
    ...
    int cnt=0,cnt2=0,l=0,r=0;
    for(int i=0;i<n;++i){
        read(a[i].fi,a[i].se);
        if(a[i].fi<0)l+=a[i].se,++cnt,L.pb({-a[i].fi,a[i].se});
        else r+=a[i].se,++cnt2,R.pb({a[i].fi,a[i].se});
    }
    sort(L.begin(),L.end()),sort(R.begin(),R.end());
    //cout<<cnt<<' '<<cnt2<<endl;

    if(cnt==cnt2)write(l+r);
    else if(cnt>cnt2){
        l=r=0;
        for(auto i:R)r+=i.se;
        for(int i=0;i<=cnt2;++i)l+=L[i].se;
        write(l+r);
    }else{
        l=r=0;
        for(auto i:L)l+=i.se;
        for(int i=0;i<=cnt;++i)r+=R[i].se;
        write(l+r);
    }
