...
int a[105],sum,dp[105][100005];
pair<int,int> kq;
int n,mn=1e9;

int _dp(int i,int j,int cnt){
    if(i==n){
        int cnt2=n-cnt;
        if(abs(cnt-cnt2)>1)return 1e9;
        if(abs((sum-j)-j)<mn){
            kq={j,sum-j};
            mn=abs((sum-j)-j);
        }
    ...
    int& ans=dp[i][j];
    if(ans!=-1)return ans;
    ans=min(_dp(i+1,j+a[i],cnt+1),_dp(i+1,j,cnt));
...
int main() {
    ...
    _dp(0,0,0);
    write(min(kq.fi,kq.se),' ',max(kq.fi,kq.se),'\n');
