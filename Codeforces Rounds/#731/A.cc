...
int main() {
    ...
    while(t--){
        ...
        if(ax==bx&&ax==cx&&cy<max(ay,by)&&cy>min(by,ay))write(abs(bx-ax)+abs(by-ay)+2,'\n');
        else if(ay==by&&ay==cy&&cx<max(ax,bx)&&cx>min(bx,ax))write(abs(bx-ax)+abs(by-ay)+2,'\n');
        else write(abs(bx-ax)+abs(by-ay),'\n');
    }
